...
public interface MovieShowsRepository extends JpaRepository<MovieShowsEntity, Integer> {
    ...
    //    https://docs.spring.io/spring-data/jpa/docs/current/reference/html/#jpa.query-methods.query-creation
       ...
       //@Query(value = "SELECT DISTINCT ON(movie_id) * FROM movie_shows WHERE show_start >= CURRENT_DATE", nativeQuery = true)
 //   @Query(value = "SELECT DISTINCT ON(ms.movie_id) * FROM movie_shows ms WHERE ms.show_start > CURRENT_DATE" , nativeQuery = true)//
	@Query(value = "SELECT *  FROM movie_shows WHERE show_start >= CURRENT_DATE", nativeQuery = true)
    List<MovieShowsEntity> findAllUpComing();

    @Query(value = "SELECT  * FROM movie_shows ms WHERE ms.show_start <= CURRENT_DATE AND ms.show_end >= CURRENT_DATE", nativeQuery = true)
    List<MovieShowsEntity> findAllNowPlaying();

    @Query(value = "SELECT  * FROM movie_shows ms WHERE ms.show_end >= CURRENT_DATE", nativeQuery = true)
    ...
    @Query(value = "SELECT  * FROM movie_shows ms WHERE ms.show_start > CURRENT_DATE LIMIT :records", nativeQuery = true)
